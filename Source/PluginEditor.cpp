...
LumiMIDIEditor::LumiMIDIEditor(LumiMIDIProcessor& p,
    ...
    : AudioProcessorEditor(&p),
      mAudioProcessor(p),
      mApvts(apvts),  // filterSection(apvts),
      midiKeyboard(keyboardState,
      ...
      mWhiteGlobalKnob(
          ...
          ParameterIDs::mainW,
          [this](double val) {
            mAudioProcessor.getAudioEngine().setGlobalWhiteLevel(val);
          }),
      mHueGlobalKnob("Hue",
                     apvts,
                     ParameterIDs::mainHue,
                     [this](double val) {
                       mAudioProcessor.getAudioEngine().setGlobalHueLevel(val);
                     }),
      mSpeedKnob("Speed",
                 apvts,
                 ParameterIDs::speed,
                 [this](double val) {
                   mAudioProcessor.getAudioEngine().setGlobalSpeedLevel(val);
                 }),
...
void LumiMIDIEditor::onSend_CC_Clicked(unsigned int cc) {
  ...
  mAudioProcessor.sendDirectMidiEvent(
...
void LumiMIDIEditor::handleNoteOn(juce::MidiKeyboardState* source,
                                  int midiChannel,
                                  int midiNoteNumber,
                                  float velocity) {
  (void)source;
...
void LumiMIDIEditor::handleNoteOff(juce::MidiKeyboardState* source,
                                   int midiChannel,
                                   int midiNoteNumber,
                                   float velocity) {
  (void)source;
...
void LumiMIDIEditor::paint(juce::Graphics& g) {
  ...
  juce::ColourGradient gradient(
      juce::Colour(0xff1a1a2e), 0, 0, juce::Colour(0xff16213e),
