...
juce::Colour oscillateColorTemperature(const juce::Colour& mainCol,
                                       float phase,
                                       float speed) {
  // Extraire la teinte (hue), saturation et luminosité de la couleur principale
  ...
  // Appliquer cette oscillation à la teinte, tout en maintenant le reste
  // constant
  ...
  // Retourner la nouvelle couleur avec la teinte ajustée
...
void WarmCoolCycle::execute(const LedVect& leds,
  ...
  // Get parameters
  const juce::uint8 mainRed =
      TO_LINE_VALUE(parameterManager.getMainRed() * MAX_CC_VALUE_F);
  const juce::uint8 mainGreen =
      TO_LINE_VALUE(parameterManager.getMainGreen() * MAX_CC_VALUE_F);
  const juce::uint8 mainBlue =
      TO_LINE_VALUE(parameterManager.getMainBlue() * MAX_CC_VALUE_F);
  ...
  const float period =
      floatToPeriod(speed) * speedFactor;  // Convert speed to period in ms
  ...
  for (const auto& led : leds) {
    events.emplace_back(led->ctrl.mr,
                        FLOAT_TO_LINE_VALUE(newCol.getFloatRed()));
    events.emplace_back(led->ctrl.mg,
                        FLOAT_TO_LINE_VALUE(newCol.getFloatGreen()));
    events.emplace_back(led->ctrl.mb,
                        FLOAT_TO_LINE_VALUE(newCol.getFloatBlue()));
