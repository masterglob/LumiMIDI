...
PageBase* PageManager::getCurrentPage() {
    switch (mCurrentPage) {
    case PageType::LedConfiguration: return &mLedConfigPage;
    case PageType::Programming: return &mProgrammingPage;
    default: return &mLedConfigPage;
    }
}

void PageManager::updateTabAppearance() {
    // Mettre à jour l'apparence des onglets selon la page active
    mTabLedConfig.setToggleState(mCurrentPage == PageType::LedConfiguration,
        juce::dontSendNotification);
    mTabProgramming.setToggleState(mCurrentPage == PageType::Programming,
        juce::dontSendNotification);
}
