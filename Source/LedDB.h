...
#define FLOAT_TO_LINE_VALUE(x) static_cast<LineValue>(MAX_CC_VALUE_F * juce::jlimit(0.0f, 0.99f, (x)))
#define TO_LINE_VALUE(x) static_cast<LineValue>(x)

/**********************************************************************************/
struct LedCtrlLine {
  LedCtrlLine(LineId r, LineId g, LineId b, LineId w) : mr(r), mg(g), mb(b), mw(w), hasWhite(true) {}
  LedCtrlLine(LineId r, LineId g, LineId b) : mr(r), mg(g), mb(b), mw(0), hasWhite(false) {}
...
struct LedPosition {
  LedPosition(const Rect& r)
      : center{r.getCentre()}, topLeft(r.getTopLeft()), size(r.getWidth(), r.getHeight()) {}
...
/**********************************************************************************/
struct LedContext {
  LedContext(const juce::String& nameRef, const LedCtrlLine& lineRef, const LedPosition& ref)
